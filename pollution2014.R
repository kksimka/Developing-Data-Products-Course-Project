...
sub <- subset(d, Parameter.Name %in% c("PM2.5 - Local Conditions", "Ozone")
              & Pollutant.Standard %in% c("Ozone 8-Hour 2008", "PM25 Annual 2013"),
              c(Longitude, Latitude, Parameter.Name, Arithmetic.Mean))

pollavg2014 <- aggregate(sub[, "Arithmetic.Mean"],
                     sub[, c("Longitude", "Latitude", "Parameter.Name")],
                     mean, na.rm = TRUE)
pollavg2014$Parameter.Name <- factor(pollavg2014$Parameter.Name, labels = c("ozone", "pm25"))
names(pollavg2014)[4] <- "level"

## Remove unneeded objects
rm(d, sub)

## Write function
monitors2014 <- data.matrix(pollavg2014[, c("Longitude", "Latitude")])
...
## lon: longitude
## lat: latitude
## radius: Radius in miles for finding monitors2014

pollutant2014 <- function(df) {
    x <- data.matrix(df[, c("lon", "lat")])
    r <- df$radius
    d <- rdist.earth(monitors2014, x)
    use <- lapply(seq_len(ncol(d)), function(i) {
        which(d[, i] < r[i])
    })
    levels <- sapply(use, function(idx) {
        with(pollavg2014[idx, ], tapply(level, Parameter.Name, mean))
    })
